...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($memberID) OR empty($memberName) OR empty($birthDate)) {
        ...
        $studyProgram = trim($_POST['prodi']);
        $classYear = trim($_POST['tahunAngkatan']);

        if ($studyProgram == '-') {
            toastr(__('Choose your study program'))->error(); //mfc
            exit();
        } 

        if (empty($classYear)) {
            toastr(__('Class Year cannot be empty'))->error(); //mfc
            exit();
        } 


        $dataMemberCollege = array();
        $dataMemberCollege['study_program'] = $studyProgram;
        $dataMemberCollege['class_year'] = $classYear;

        
        if($_POST['updateRecordMemberCollegeID'] <= 0){
           // print_r($dataMemberCollege);
            $sql_op->insert('member_college', $dataMemberCollege);
        } 
        ...
        $data['member_college_id'] = $sql_op->insert_id;
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            $updateRecordMemberCollegeID = $_POST['updateRecordMemberCollegeID'];
             if ($_POST['updateRecordMemberCollegeID'] >= 1) {
                $updateMemberCollege = $sql_op->update('member_college', $dataMemberCollege, "id='$updateRecordMemberCollegeID'");
             }
            
            // update the data
            $update = $sql_op->update('member', $data, "member_id='$updateRecordID'");
            if ($update || $updateMemberCollege) {
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        $loan_q = $dbs->query('SELECT DISTINCT m.member_id, m.member_college_id, m.member_name, COUNT(l.loan_id) FROM member AS m
        ...
        if ($loan_d[3] < 1) {
            if (!$sql_op->delete('member_college', "id='$loan_d[1]'")) {
                $error_num++;
            } else {
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'membership', $_SESSION['realname'].' DELETE member data ('.$loan_d[2].') with ID ('.$loan_d[0].')', 'Delete', 'OK');
            }
            ...
            $still_have_loan[] = $loan_d[0].' - '.$loan_d[2];
            $error_num++;
        }
    }
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    if (!empty($itemID) && $rec_q->num_rows > 0) {
        ...
        $memberCollegeId = (integer)$rec_d['member_college_id'];
        $form->addHidden('updateRecordMemberCollegeID', $memberCollegeId);
    ...
    //prodi
    $prodiOptions = array();
    $prodiOptions[] = array('-', __('Pilih'));
    $prodiOptions[] = array('TEO', __('Teologi'));
    $prodiOptions[] = array('PAK', __('Pendidikan Agama Kristen'));
    $form->addSelectList('prodi', __('Program Studi').'*', $prodiOptions, '','class="form-control col-4" style="display: none;"');

    //tahun angkatan
    $form->addTextField('text', 'tahunAngkatan', __('Tahun Angkatan').'*', '', 'class="form-control" style="display: none; width: 20%;"');
...
$(document).ready(function() {
    ...
    $("#simbioFormRowprodi").hide();
    $("#simbioFormRowtahunAngkatan").hide();

    $("#memberTypeID").change(function() {
        // If the user selects "Other", show the input field
        if ($(this).val() == 5) {
            $("#simbioFormRowprodi").show();
            $("#prodi").show();
            $("#simbioFormRowtahunAngkatan").show();
            $("#tahunAngkatan").show();        
        } else {
            $("#simbioFormRowprodi").hide();
            $("#prodi").hide();
